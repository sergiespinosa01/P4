...
for cmd in $*; do
   ...
   if [[ $cmd == train ]]; then
       ...
       for dir in $db/BLOCK*/SES* ; do
           name=${dir/*\/}
           echo $name ----
           gmm_train  -v 1 -T 0.001 -N10 -m 6 -d $w/$FEAT -e $FEAT -g $w/gmm/$FEAT/$name.gmm $lists/class/$name.train || exit 1
           echo
       done
   elif [[ $cmd == test ]]; then
       ...
       gmm_train  -v 1 -T 0.001 -N10 -m 6 -d $w/$FEAT -e $FEAT -g $w/gmm/$FEAT/$world.gmm $lists/verif/$world.train || exit 1
